...
- 今日行动档案也会读取 `bootcamp_sessions.parsed_profile.projects[].targetEvidence`，识别“目标证据已补但还没入画像账本”的项目，并优先提示“现在入账这份证据”；故事库保存项目故事包时会把 `targetEvidence` 一并写入 `growth_snapshots.dimension_scores.__trigger.projectStory`，Dashboard 证据账本刷新后可读回这段目标证据。
