...
test("extracts block style generated question reason", () => {
  const result = parseGeneratedQuestionText(`【为什么练这题】你需要训练机会成本量化，而不是只凭直觉排序。

题目正文：你负责一款企业文档工具，搜索优化和企微集成都无法并行。请计算显性机会成本和隐性成本，并给出取舍。`);

  assert.equal(result.reason, "你需要训练机会成本量化，而不是只凭直觉排序。");
